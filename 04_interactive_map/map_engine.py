...
class GeneradorMapas:
    ...
    def crear_popup_html(self, region: str, stats: Dict) -> str:
        """Crea HTML para popup con estadísticas de la región"""
        
        # Top 3 tipos de conflicto
        top_conflictos = sorted(
            stats['tipos_conflicto'].items(),
            key=lambda x: x[1],
            reverse=True
        )[:3]
        ...
        <div style="font-family: Arial; width: 350px; max-height: 400px; overflow-y: auto;">
            <h3 style="color: #2c3e50; margin-bottom: 10px;">{region}</h3>
            <hr style="margin: 10px 0;">
            
            <p style="font-size: 14px; margin: 5px 0;">
                <strong>Total conflictos:</strong> {stats['total_conflictos']}
            </p>
            
            <p style="font-size: 12px; margin-top: 10px; margin-bottom: 5px;">
                <strong>Tipos de conflicto principales:</strong>
            </p>
            <ul style="font-size: 11px; margin: 0; padding-left: 20px;">
        """
        
        for tipo, cantidad in top_conflictos:
            if tipo:
                html += f"<li>{tipo}: {cantidad}</li>"
        ...
        for sector, cantidad in top_sectores:
            if sector:
                html += f"<li>{sector}: {cantidad}</li>"
        
        html += """
            </ul>
            
            <p style="font-size: 12px; margin-top: 15px; margin-bottom: 5px;">
                <strong>📰 Noticias (últimas 5):</strong>
            </p>
            <div style="font-size: 10px; margin: 0; padding: 5px; background: #f8f9fa; border-radius: 5px; max-height: 150px; overflow-y: auto;">
        """
        
        # Agregar últimas 5 noticias con enlaces
        noticias = stats.get('noticias', [])[:5]
        for i, noticia in enumerate(noticias, 1):
            titulo = noticia.get('titulo', 'Sin título')[:80]
            url = noticia.get('url', '#')
            fecha = noticia.get('fecha', 'Sin fecha')
            
            # Truncar título si es muy largo
            if len(noticia.get('titulo', '')) > 80:
                titulo += '...'
            
            html += f"""
                <p style="margin: 5px 0; padding: 5px; background: white; border-left: 3px solid #3498db;">
                    <strong>{i}.</strong> 
                    <a href="{url}" target="_blank" style="color: #2980b9; text-decoration: none;">
                        {titulo}
                    </a>
                    <br>
                    <span style="color: #7f8c8d; font-size: 9px;">📅 {fecha}</span>
                </p>
            """
    ...
    def crear_popup_html_detallado(self, ubicacion: str, stats: Dict) -> str:
        """
        Crea HTML detallado para popup con panel lateral interactivo
        Muestra información completa de cada noticia sin redirección externa
        """
        html = f"""
        <div style="font-family: Arial; width: 400px; max-height: 500px; overflow-y: auto;">
            <h3 style="color: #2c3e50; margin-bottom: 10px;">{ubicacion}</h3>
            <hr style="margin: 10px 0;">
            ...
            <p style="font-size: 12px; margin-top: 15px; margin-bottom: 5px;">
                <strong>📰 Noticias Detalladas:</strong>
            </p>
            <div style="font-size: 11px; margin: 0; padding: 5px; background: #f8f9fa; border-radius: 5px; max-height: 350px; overflow-y: auto;">
        """
        
        # Agregar todas las noticias con información detallada
        noticias = stats.get('noticias', [])
        for i, noticia in enumerate(noticias, 1):
            titulo = noticia.get('titulo', 'Sin título')
            fecha = noticia.get('fecha', 'Sin fecha')
            resumen = noticia.get('resumen', 'Sin resumen')
            tipo_conflicto = noticia.get('tipo_conflicto', '')
            explicacion_conflicto = noticia.get('explicacion_conflicto', '')
            tipo_accion = noticia.get('tipo_accion', '')
            explicacion_accion = noticia.get('explicacion_accion', '')
            actor_demandante = noticia.get('actor_demandante', '')
            explicacion_demandante = noticia.get('explicacion_demandante', '')
            actor_demandado = noticia.get('actor_demandado', '')
            explicacion_demandado = noticia.get('explicacion_demandado', '')
            region = noticia.get('region', '')
            provincia = noticia.get('provincia', '')  # ✅ AGREGADO
            comuna = noticia.get('comuna', '')
            localidad = noticia.get('localidad', '')
            sector = noticia.get('sector_economico', '')
            url = noticia.get('url', '#')
            
            # Construir ubicación completa (convertir a string y filtrar NaN/None)
            ubicacion_completa = []
            if region and str(region) != 'nan':
                ubicacion_completa.append(str(region))
            if provincia and str(provincia) != 'nan':  # ✅ AGREGADO
                ubicacion_completa.append(str(provincia))
            if comuna and str(comuna) != 'nan':
                ubicacion_completa.append(str(comuna))
            if localidad and str(localidad) != 'nan':
                ubicacion_completa.append(str(localidad))
            ubicacion_str = ' → '.join(ubicacion_completa) if ubicacion_completa else 'No especificada'
            
            html += f"""
                <div style="margin: 10px 0; padding: 10px; background: white; border-left: 4px solid #3498db; border-radius: 3px;">
                    <h4 style="margin: 0 0 8px 0; color: #2c3e50; font-size: 13px;">
                        {i}. {titulo}
                    </h4>
                    <p style="margin: 5px 0; color: #7f8c8d; font-size: 10px;">
                        📅 {fecha}
                    </p>
                    
                    <p style="margin: 8px 0; font-size: 11px; color: #34495e; line-height: 1.4;">
            ...
            if tipo_conflicto:
                html += f"""
                    <p style="margin: 5px 0; font-size: 10px;">
                        <strong style="color: #e74c3c;">🔴 Tipo de Conflicto:</strong> {tipo_conflicto}
                """
                if explicacion_conflicto:
                    html += f"""<br><span style="color: #7f8c8d; font-size: 9px;">{explicacion_conflicto}</span>"""
                html += "</p>"
            ...
            if tipo_accion:
                html += f"""
                    <p style="margin: 5px 0; font-size: 10px;">
                        <strong style="color: #f39c12;">⚡ Tipo de Acción:</strong> {tipo_accion}
                """
                if explicacion_accion:
                    html += f"""<br><span style="color: #7f8c8d; font-size: 9px;">{explicacion_accion}</span>"""
                html += "</p>"
            ...
            if actor_demandante:
                html += f"""
                    <p style="margin: 5px 0; font-size: 10px;">
                        <strong style="color: #3498db;">👥 Actor Demandante:</strong> {actor_demandante}
                """
                if explicacion_demandante:
                    html += f"""<br><span style="color: #7f8c8d; font-size: 9px;">{explicacion_demandante}</span>"""
                html += "</p>"
            
            if actor_demandado:
                html += f"""
                    <p style="margin: 5px 0; font-size: 10px;">
                        <strong style="color: #9b59b6;">🏢 Actor Demandado:</strong> {actor_demandado}
                """
                if explicacion_demandado:
                    html += f"""<br><span style="color: #7f8c8d; font-size: 9px;">{explicacion_demandado}</span>"""
                html += "</p>"
            
            # Ubicación y sector
            html += f"""
                    <p style="margin: 5px 0; font-size: 10px;">
                        <strong style="color: #16a085;">📍 Ubicación:</strong> {ubicacion_str}
                    </p>
            """
            ...
            if sector:
                html += f"""
                    <p style="margin: 5px 0; font-size: 10px;">
                        <strong style="color: #27ae60;">🏭 Sector:</strong> {sector}
                    </p>
                """
            ...
            if url and url != '#':
                html += f"""
                    <p style="margin: 8px 0 0 0; text-align: right;">
                        <a href="{url}" target="_blank" style="color: #2980b9; text-decoration: none; font-size: 10px; font-weight: bold;">
    ...
    def generar_mapa_regiones(self, df: pd.DataFrame, output_path: str = None) -> folium.Map:
        """
        Genera mapa de calor por regiones
        ...
        Returns:
            Objeto folium.Map
        """
        logger.info("Generando mapa de calor por regiones...")
        ...
        # Procesar datos
        stats = self.procesar_datos_excel(df)
        
        if not stats:
            logger.warning("No hay datos para generar mapa")
            return mapa
        
        # Cargar GeoJSON de regiones
        geojson_data = self.cargar_geojson('regiones')
        
        if not geojson_data:
            logger.error("No se pudo cargar GeoJSON de regiones")
            return mapa
        
        # Calcular máximo de conflictos para escala de colores
        max_conflictos = max([s['total_conflictos'] for s in stats.values()])
        
        # Agregar capa de coropletas (mapa de calor)
        for feature in geojson_data['features']:
            region_nombre_geojson = feature['properties'].get('Region')
            # Normalizar nombre del GeoJSON para comparación
            ...
            if region_nombre_normalizado in stats:
                ...
                # Crear polígono con color (usar nombre original para display)
                folium.GeoJson(
                    feature,
        ...
        <div style="position: fixed; 
            ...
            <p style="margin: 5px 0;"><span style="background-color: #F03B20; padding: 5px 10px;">█</span> Alta (75-100%)</p>
            <p style="margin: 5px 0;"><span style="background-color: #FEB24C; padding: 5px 10px;">█</span> Media-Alta (50-75%)</p>
            <p style="margin: 5px 0;"><span style="background-color: #FFEDA0; padding: 5px 10px;">█</span> Media (25-50%)</p>
            <p style="margin: 5px 0;"><span style="background-color: #FFFFCC; padding: 5px 10px;">█</span> Baja (0-25%)</p>
        </div>
        """
    ...
    def generar_mapa_comunal(self, df: pd.DataFrame, region_filtro: str = None, output_path: str = None) -> folium.Map:
        """
        Genera mapa de calor por comunas
        
        Args:
            df: DataFrame con noticias filtradas
            region_filtro: Filtrar por región específica (opcional)
            output_path: Ruta para guardar HTML (opcional)
        
        Returns:
            Objeto folium.Map
        """
        logger.info("Generando mapa de calor por comunas...")
        ...
        # NORMALIZAR nombres antes de filtrar
        if 'region' in df_normalizado.columns:
            df_normalizado['region'] = df_normalizado['region'].apply(lambda x: self.normalizar_nombre_geografico(x, nivel='regiones'))
        if 'comuna' in df_normalizado.columns:
            df_normalizado['comuna'] = df_normalizado['comuna'].apply(lambda x: self.normalizar_nombre_geografico(x, nivel='comunas'))
        ...
        if region_filtro:
            region_filtro_normalizado = self.normalizar_nombre_geografico(region_filtro, nivel='regiones')
            df_normalizado = df_normalizado[df_normalizado['region'] == region_filtro_normalizado]
        
        # Crear mapa base
        mapa = folium.Map(
            location=self.chile_center,
            zoom_start=self.chile_zoom if not region_filtro else 7,
            tiles='OpenStreetMap'
        )
        ...
        for comuna in df_normalizado['comuna'].dropna().unique():
            ...
            # Preparar lista de noticias con información completa
            noticias_list = []
            for _, row in df_comuna.iterrows():
                noticias_list.append({
                    ...
                    'url': row.get('link_noticia', row.get('url', '#')),
                    'fecha': str(row.get('fecha', 'Sin fecha'))[:10],
                    'fecha_raw': row.get('fecha'),  # Para ordenamiento
                    'resumen': row.get('resumen', ''),
                    'tipo_conflicto': row.get('tipo_conflicto', ''),
                    'tipo_accion': row.get('tipo_accion', ''),
                    'explicacion_conflicto': row.get('explicacion_conflicto', ''),
            ...
            stats_comunas[comuna] = {
                'total_conflictos': len(df_comuna),
                'tipos_conflicto': df_comuna['tipo_conflicto'].value_counts().to_dict() if 'tipo_conflicto' in df_comuna.columns else {},
                'sectores': df_comuna['sector_economico'].value_counts().to_dict() if 'sector_economico' in df_comuna.columns else {},
                'noticias': noticias_list
            }
        
        if not stats_comunas:
            logger.warning("No hay datos comunales para generar mapa")
            return mapa
        
        # Cargar GeoJSON de comunas
        geojson_data = self.cargar_geojson('comunas')
        
        if not geojson_data:
            logger.error("No se pudo cargar GeoJSON de comunas")
            return mapa
        
        # Calcular máximo de conflictos
        max_conflictos = max([s['total_conflictos'] for s in stats_comunas.values()])
        
        # Agregar capa de comunas
        for feature in geojson_data['features']:
            comuna_nombre_geojson = feature['properties'].get('Comuna')
            # Normalizar nombre del GeoJSON para comparación
            comuna_nombre_normalizado = self.normalizar_nombre_geografico(comuna_nombre_geojson, nivel='comunas')
            
            if comuna_nombre_normalizado in stats_comunas:
                cantidad = stats_comunas[comuna_nombre_normalizado]['total_conflictos']
                color = self.calcular_intensidad_color(cantidad, max_conflictos)
                
                # Usar nombre original del GeoJSON para display
                folium.GeoJson(
                    feature,
                    style_function=lambda x, color=color: {
                        'fillColor': color,
        ...
        if output_path:
            mapa.save(output_path)
            logger.info(f"Mapa comunal guardado en: {output_path}")
        
        return mapa
    
    def generar_mapa_provincial(self, df: pd.DataFrame, region_filtro: str = None, output_path: str = None) -> folium.Map:
        """
        Genera mapa de calor por provincias
        ...
        Returns:
            Objeto folium.Map
        """
        logger.info("Generando mapa de calor por provincias...")
        ...
        for provincia in df_normalizado['provincia'].dropna().unique():
            ...
            # Preparar lista de noticias con URL
            noticias_list = []
            ...
            stats_provincias[provincia] = {
                'total_conflictos': len(df_provincia),
                'tipos_conflicto': df_provincia['tipo_conflicto'].value_counts().to_dict() if 'tipo_conflicto' in df_provincia.columns else {},
                'sectores': df_provincia['sector_economico'].value_counts().to_dict() if 'sector_economico' in df_provincia.columns else {},
                'noticias': noticias_list
            }
        
        # Cargar GeoJSON de provincias
        geojson_data = self.cargar_geojson('provincias')
        
        if not geojson_data:
            logger.error("No se pudo cargar GeoJSON de provincias")
            return mapa
        
        # Calcular máximo de conflictos para escala de colores
        if stats_provincias:
            max_conflictos = max([s['total_conflictos'] for s in stats_provincias.values()])
        else:
            logger.warning("No hay datos de provincias para mostrar")
            return mapa
        
        # Agregar capas de provincias
        for feature in geojson_data['features']:
            provincia_nombre_geojson = feature['properties'].get('NOM_PROV')
            # Normalizar nombre del GeoJSON para comparación
            provincia_nombre_normalizado = self.normalizar_nombre_geografico(provincia_nombre_geojson, nivel='provincias')
            
            if provincia_nombre_normalizado in stats_provincias:
                cantidad = stats_provincias[provincia_nombre_normalizado]['total_conflictos']
                color = self.calcular_intensidad_color(cantidad, max_conflictos)
        ...
        if output_path:
            mapa.save(output_path)
            logger.info(f"Mapa provincial guardado en: {output_path}")
